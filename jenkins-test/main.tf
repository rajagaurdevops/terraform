...
resource "aws_instance" "web" {
  ...
  user_data = <<-EOF
              #!/bin/bash
              set -ex  # Enable debugging and stop script on error

              # Update system
              apt update -y
              apt upgrade -y
              sleep 10  # Pause to ensure updates are applied
              ...
              systemctl daemon-reload  # Reload system daemons
...
resource "aws_security_group" "web_sg" {
  ...
  ingress {
    from_port   = 8080  # Allow Jenkins Web UI access
    to_port     = 8080
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  }
